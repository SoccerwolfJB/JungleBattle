...
public class CircleShield extends CustomEffect {
	...
	public void runEffect(Player p) {
		Location l = p.getEyeLocation().clone(); //Klone EyeLocation des Spielers
		...
		while (factor <= 1D) { //während Faktor kleiner oder gleich 1 ist, führe Folgendes aus:
			double xfac = Math.sin(Math.toRadians(360 * factor)); //Berechne X-Koordinaten-Faktor des Effekts
			double zfac = Math.cos(Math.toRadians(360 * factor)); //Berechne Z-Koordinaten-Faktor des Effekts
			...
			for (double y = 0; y < 1.8; y += 0.2) { //9-malige Ausführung
				...
				if(water) p.getWorld().playEffect(l, Effect.WATERDRIP, 0); //Selbsterklärend
				else p.getWorld().playEffect(l, Effect.LAVADRIP, 0);
			}
